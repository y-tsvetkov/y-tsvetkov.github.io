...
## Software
This is the meat and potatoes of the project. The approach was to train a network that does periodic signal in simulation using CMA-ES and then just transfer that to the real world.
...
### Reward Function
At first, I tried just using the distance travelled as the fitness function, but was having trouble with the agent just falling forwards:
...
### Training
I made an URDF of the bot using the inertial data from Fusion 360 and a PyBullet-based environment compatible with the [OpenAI Gym](https://github.com/openai/gym) framework (this was before it was discontinued). In order to make the policy robust to different conditions, I randomised a bunch of things, here's the slide about them from my presentation on the bot: 
...
The custom PDB is a pretty standard board for distributing power, aside from one neat thing. The SBEC came with dual output leads, each of which ended with a generic 3-pin servo connector with a power, ground and an empty slot. I had the PDB power input pins next to each other, with the empty pins on both ends. If one connector was flipped, only a single wire would be connected to a pin and the circuit will remain open, meaning it's not possible to short the circuit and brick everything.
<div class="row">
    <div class="col-sm mt-3 mt-md-0">
